...
function TicketDetail({ ticketId, setCurrentPage }) {
  ...
  const engineerActions = [
    { label: "Accept", status: "Assigned", icon: UserCheck, variant: "outline" },
    { label: "Start Work", status: "In Progress", icon: PlayCircle, variant: "outline" },
    { label: "Completed", status: "Completed", icon: CheckCircle, variant: "outline" },
    { label: "Close", status: "Closed", icon: Archive, variant: "outline" },
  ];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{ticket.problem_title}</p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <div className="lg:col-span-2">
              <Card className="border-0 shadow-sm">
                <CardContent className="pt-6">
                  <h6 className="text-sm font-semibold mb-3">Ticket Details</h6>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                    <div>
                      <span className="font-medium">Requested By:</span> {ticket.requested_by}
                    </div>
                    <div>
                      <span className="font-medium">Email:</span> {ticket.requested_by_email || "-"}
                    </div>
                    <div>
                      <span className="font-medium">Phone:</span> {ticket.requested_by_phone || "-"}
                    </div>
                    <div>
                      <span className="font-medium">Created:</span> {ticket.created_at ? new Date(ticket.created_at).toLocaleString() : "-"}
                    </div>
                    <div>
                      <span className="font-medium">Category ID:</span> {ticket.category_id || "-"}
                    </div>
                    <div>
                      <span className="font-medium">Department ID:</span> {ticket.department_id || "-"}
                    </div>
                    <div>
                      <span className="font-medium">Location ID:</span> {ticket.location_id || "-"}
                    </div>
                    <div>
                      <span className="font-medium">Asset ID:</span> {ticket.asset_id || "-"}
                    </div>
                    <div>
                      <span className="font-medium">Cost:</span> {ticket.cost != null ? `\u20B9${Number(ticket.cost).toLocaleString()}` : "-"}
                    </div>
                    <div>
                      <span className="font-medium">Spare Parts:</span> {ticket.spare_parts || "-"}
                    </div>
                    <div className="col-span-full">
                      <span className="font-medium">Description:</span>
                      <p className="mt-1 text-muted-foreground">{ticket.problem_description || "No description provided."}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="space-y-4">
              <Card className="border-0 shadow-sm">
                <CardContent className="pt-6">
                  ...
                  <h6 className="text-sm font-semibold mb-2">Assignment</h6>
                  ...
                  <h6 className="text-sm font-semibold mb-2 mt-4">Cost & Parts</h6>
                  <div className="space-y-2">
                    <div className="flex gap-2">
                      <Input
                        type="number"
        ...
        <TabsContent value="comments">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2">
              <Card className="border-0 shadow-sm">
                <CardContent className="pt-6">
                  <h6 className="text-sm font-semibold mb-3">Comments</h6>
                  {comments.length === 0 ? (
            ...
            <div>
              <Card className="border-0 shadow-sm">
                <CardContent className="pt-6">
                  <h6 className="text-sm font-semibold mb-2">Add Comment</h6>
                  <div className="space-y-2">
                    <Input
                      value={commentUser}
        ...
        <TabsContent value="photos">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2">
              <Card className="border-0 shadow-sm">
                <CardContent className="pt-6">
                  <h6 className="text-sm font-semibold mb-3">Photos</h6>
                  {photos.length === 0 ? (
                    <p className="text-sm text-muted-foreground">No photos uploaded yet.</p>
                  ) : (
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {photos.map((ph) => (
                        <div key={ph.id} className="border rounded-lg p-3 text-center">
                          <Image className="h-8 w-8 text-muted-foreground mx-auto" />
                          <div className="mt-1 text-sm font-semibold">{ph.file_name}</div>
            ...
            <div>
              <Card className="border-0 shadow-sm">
                <CardContent className="pt-6">
                  <h6 className="text-sm font-semibold mb-2">Upload Photo</h6>
                  <div className="space-y-3">
                    ...
                    <Button size="sm" className="w-full" onClick={handleAddPhoto}>
        ...
        <TabsContent value="timeline">
          <Card className="border-0 shadow-sm">
            <CardContent className="pt-6">
              <h6 className="text-sm font-semibold mb-3">Timeline</h6>
              {history.length === 0 ? (
                  ...
                  {history.map((h) => (
                    <div key={h.id} className="flex gap-3 items-start">
                      ...
                      <div className="flex-1">
                        ...
                        <div><span className="text-xs text-muted-foreground">By: {h.performed_by}</span></div>
                      </div>
                    </div>
                  ))}
                </div>
